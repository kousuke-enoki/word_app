...
func (f JapaneseMeanFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.JapaneseMeanMutation); ok {
		return f(ctx, mv)
	}
...
// The WordInfoFunc type is an adapter to allow the use of ordinary
// function as WordInfo mutator.
type WordInfoFunc func(context.Context, *ent.WordInfoMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f WordInfoFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.WordInfoMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.WordInfoMutation", m)
}
