...
func (qq *QuizQuery) QueryQuizQuestions() *QuizQuestionQuery {
	query := (&QuizQuestionClient{config: qq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (qq *QuizQuery) WithQuizQuestions(opts ...func(*QuizQuestionQuery)) *QuizQuery {
	query := (&QuizQuestionClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qq.withQuizQuestions = query
	return qq
}
...
func (qq *QuizQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Quiz, error) {
	...
	if query := qq.withQuizQuestions; query != nil {
		if err := qq.loadQuizQuestions(ctx, query, nodes,
			func(n *Quiz) { n.Edges.QuizQuestions = []*QuizQuestion{} },
			func(n *Quiz, e *QuizQuestion) { n.Edges.QuizQuestions = append(n.Edges.QuizQuestions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (qq *QuizQuery) loadQuizQuestions(ctx context.Context, query *QuizQuestionQuery, nodes []*Quiz, init func(*Quiz), assign func(*Quiz, *QuizQuestion)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(quizquestion.FieldQuizID)
	}
	query.Where(predicate.QuizQuestion(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(quiz.QuizQuestionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (qq *QuizQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := qq.ctx.Fields; len(fields) > 0 {
		...
		if qq.withUser != nil {
			_spec.Node.AddColumnOnce(quiz.FieldUserID)
		}
	}
