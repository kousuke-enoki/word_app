...
func TestRegisterWordHandler(t *testing.T) {
	...
	t.Run("GetRegisteredWords_success", func(t *testing.T) {
		...
		mockService.On("RegisterWords", mock.Anything, mock.Anything).Return(&models.RegisterWordResponse{
		...
		// リクエスト準備
		req, _ := http.NewRequest("POST", "/words/register", nil)
	...
	// 	router.GET("/words/:id", wordHandler.RegisterWordHandler())
