...
func (s *ExamServiceImpl) GetExam(ctx context.Context, req *models.GetExamRequest) (*models.GetExamResponse, error) {
	// userID := req.UserID
	// examID := req.ExamID
	...
	// // 	}
	// // }()
	// exam, err := s.client.Exam.
	// 	Query().
	// 	Where(exam.ID(examID)).
	// 	WithExamQuestions(func(wq *ent.ExamQuestionQuery) {
	// 		wq.WithJapaneseMeans().WithPartOfSpeech()
	// 	}).
	// 	WithRegisteredWords().
	// 	Only(ctx)
	// exam, err := s.client.Exam.Get(ctx, examID)
	// // SetUserID(userID).
	...
	// // Save(ctx)
	// if err != nil {
	// 	return nil, errors.New("failed to fetch exam")
	// }
