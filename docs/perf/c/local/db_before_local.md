...
| ------------------- | -------: | -------: | -------: | ---------: | -------: | :-: |
| GET /words?search=  |    **73** |  **157** |    **-** |  **0.00%** | **4.10** | ✅  |
...
- **SLO 達成**: p95 latency が **156.94ms** で 200ms を下回っており、SLO を達成しています
- **エラー率**: **0.00%** で安定しています
- **平均レイテンシ**: **84.12ms** で、検索処理としては良好な性能です
- **中央値（p50）**: **72.64ms** と低く、大半のリクエストが高速に処理されています
- **最大レイテンシ**: **607.55ms** は散発的に発生していますが、p95 が 200ms を下回っているため許容範囲内です
