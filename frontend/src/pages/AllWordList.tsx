...
const AllWordList: React.FC = () => {
  const [words, setWords] = useState<WordInfo[]>([]); // Word[] 型の単語リスト
    ...
    <div>
      ...
      <table>
        ...
        <tbody>
          {words.map((word) => (
            <tr key={word.id}>
              <td>{word.edges.word.name}</td>
              <td>{word.edges.japanese_means.map(mean => mean.meaning).join(', ')}</td>
              <td>{word.edges.word_info.part_of_speech}</td>
