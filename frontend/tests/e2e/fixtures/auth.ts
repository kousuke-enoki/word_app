  ...
  expectRoleProtectedNavigation: async ({ page, signInAs }, use) => {
    ...
    await use(async (role: UserRole) => {
      ...
      for (const testCase of testCases) {
        ...
        if (testCase.allowed) {
          await expect(page).toHaveURL(`${APP_ORIGIN}${testCase.path}`)
          await testCase.assertion()
        } else {
          // 権限がない場合、PrivateRouteが/にリダイレクトし、
          // PublicRouteが再度/mypageにリダイレクトする
          await expect(page).toHaveURL(`${APP_ORIGIN}/mypage`)
        }
      }
    })
  },
