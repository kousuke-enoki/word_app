...
export class AppStack extends Stack {
  constructor(
    ...
    const imageTagOrDigest =
      this.node.tryGetContext('imageDigest') ?? this.node.tryGetContext('imageTag') ?? 'lambda'
    ...
    const fn = new lambda.DockerImageFunction(this, 'ApiFn', {
      ...
      reservedConcurrentExecutions: 20, // 予約同時実行数
    ...
    const errorAlarm = new cloudwatch.Alarm(this, 'Api5xxAlarm', {
      ...
      alarmDescription: 'Alarm when API Gateway 5xx errors exceed 50 in 5 minutes',
    })
